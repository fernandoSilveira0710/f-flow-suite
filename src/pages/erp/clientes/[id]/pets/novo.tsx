...
export default function NovoPet() {
  ...
  const loadData = async (customerId: string) => {
    try {
      setLoadingData(true);
      const [customerData, speciesData] = await Promise.all([
        fetchCustomer(customerId),
        fetchPetSpecies()
      ]);
      setCustomer(customerData);
      setSpecies(speciesData);
      setFormData(prev => ({ ...prev, tutorId: customerId }));
    } catch (error) {
  ...
  const handleInputChange = (field: keyof CreatePetDto, value: string | boolean | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
